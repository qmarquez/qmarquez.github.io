...
export default ({ locale, skills }) => {
  const mapSkill = (skill, i) => {
    ...
    return (
      <div key={i} className="section-tech-skill-container">
        <div className="tech-skill-title">{title}</div>
        {
          time && <div>
            <Icon className="section-work-activity" icon="business-time" />
        ...
        }
        {
          details.map(
            ({ text, icon }, j) => (
              <div key={j}>
                <Icon className="section-work-activity" icon={icon} />{text}
              </div>
            )
          )
        }
